    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-body p-4">
                <form method='post' action="{{url('/action/user-edit')}}" id="user-edit-form">
                @csrf
                    <input type="hidden" class="form-control" name="id" id="id" value="{{ $selected_user->id }}">
                    <div class="mb-3 form-group">
                        <label class="form-label" for="name">Full Name</label>
                        <input type="text" class="form-control" name="name" id="name" placeholder="Enter Full Name" value="{{ $selected_user->name }}">
                    </div>
                    <div class="mb-3 form-group">
                        <label class="form-label" for="address">Address</label>
                        <input type="text" class="form-control" id="address" name="address" placeholder="Enter Address" value="{{ $selected_user->address }}">
                    </div>
                    <div class="row"> 
                        <div class="col-md-12">
                            <div class="mb-3 form-group">
                                ...
                                <select class="form-select" id="user_role" name="user_role">
                                    ...
                                    @foreach($user_role as $key=>$data)
                                        <option <?php if($selected_user->user_role == $data->id){ echo 'selected'; } ?> value="{{ $data->id }}">{{ $data->user_role }}</option>  
                                    @endforeach
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="row">  
                        <div class="col-md-6">
                            <div class="mb-3 form-group">
                                ...
                                <input type="text" class="form-control" id="nic" name="nic" placeholder="Enter NIC" value="{{ $selected_user->nic }}">
                            </div>
                        </div>                                                          
                        <div class="col-md-6">
                            <div class="mb-3 form-group">
                                <label class="form-label" for="email">Email</label>
                                <input type="email" class="form-control" id="email" name="email" placeholder="Enter your Email" value="{{ $selected_user->email }}">
                            </div>
                        </div>
                    </div>  
